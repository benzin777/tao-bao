...
| Goals | Grammarly Goals | Audience, formality, domain, intent influence suggestions. | Do not expose goal controls as setup. Tao Dao infers enrichment after formula fit. |
...
| Prompt commands | Scramble | Reusable text actions such as fix, improve, simplify, expand. | Transform these into automatic rewrite options, not free-floating writing utilities. |
...
| In-place commands | QuillBot | Correction plus contextual commands: rewrite, expand, simplify, change tone, preserve workflow. | Do not make paraphrasing the core. Rewrite options are secondary after formula fit. |
